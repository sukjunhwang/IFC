...
def video_generalized_box_iou(out_bbox, tgt_bbox, valid):
    """
    out_bbox: [N, num_frames, 4]
    tgt_bbox: [M, num_frames, 4]
    valid: [M, num_frames]
    ...
    iou, union = video_box_iou(out_bbox, tgt_bbox)  # [N, M, num_frames] respectively

    lt = torch.min(out_bbox[:, None, :, :2], tgt_bbox[:, :, :2])
    rb = torch.max(out_bbox[:, None, :, 2:], tgt_bbox[:, :, 2:])

    wh = (rb - lt).clamp(min=0)  # [N, M, num_frames, 2]
    area = wh[:, :, :, 0] * wh[:, :, :, 1]  # [N, M, num_frames]

    giou = iou - (area - union) / area      # [N, M, num_frames]
    giou[:, ~valid] = 0.0

    return giou.sum(dim=-1) / valid.float().sum(dim=-1)
